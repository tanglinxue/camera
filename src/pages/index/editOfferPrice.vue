  ...
  <view class="main column-center">
    <view class="boxInner box column-center mgb50">
      <view class="top-title row-start">
        ...
        <view class="input-box row-center"
          ><input
            type="text"
            placeholder="请输入活动主题"
            class="input"
            v-model="query.case_name"
        /></view>
      </view>
      <view class="row-between top-title">
        <view class="row-start">
          <view class="label">合作方：</view>
          <view class="input-box row-center width200"
            ><input
              type="text"
              placeholder="合作方"
              class="input"
              v-model="query.case_host"
          /></view>
        </view>
        <view class="row-start mgl20">
          <view class="label">联系人：</view>
          <view class="input-box row-center width160"
            ><input
              type="text"
              placeholder="联系人"
              class="input"
      ...
      <view
        v-for="(item, index) in inerAllPro"
        :key="index"
        style="width: 100%"
      >
        <template v-if="item.list.length">
          ...
          <view v-for="bitem in item.list" :key="bitem.id">
            <view class="row row-between">
              ...
              <view
                class="right-txt row-start"
                @click="openTag(bitem, bitem.id)"
                v-if="!bitem.noNum || !bitem.noDays"
              >
                ￥{{ bitem.unit_price }}
                <template v-if="!bitem.noNum">
                  *{{ bitem.num }}{{ bitem.unit }}
                </template>
                <template v-if="!bitem.noDays"> *{{ bitem.days }}天 </template>
                <template> ={{ bitem.price }}元 </template>
                <uni-icons
                  type="bottom"
                  size="15"
                  color="#919191"
            ...
            <template
              v-if="
                idArrs.includes(bitem.id) && (!bitem.noNum || !bitem.noDays)
              "
            >
      ...
      <view class="other-pro mgb20" v-for="(item, index) in list" :key="index"
        ><reduceCom :info="item" :deleteIcon="true" :nodeid="8"></reduceCom
      ></view>
      ...
      <view class="total-price weight row-end">
        ...
        ：
        <view class="input-box row-center width160"
          ><input type="number" class="input" v-model="query.real_money"
        /></view>
      </view>
    ...
    <view class="row-center btn-box"
      ><view class="main-btn" @click="submit">保存</view></view
    >
    <changePopupEdit />
  </view>
</template>
...
export default {
  ...
  methods: {
    ...
    async submit() {
      ...
      let API = price_id
        ? this.$API.home.edit_price
        : this.$API.home.publish_price;
