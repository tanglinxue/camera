<template>
	<view class="main column-center">
		...
		<button class="main-btn mgb40" open-type="getPhoneNumber" @getphonenumber="getPhoneNumber">获取手机号</button>
		...
		methods: {
			...
			async tologin() {
				...
				let {
					...
					encryptedData,
					errMsg
				} = await this.$methods.getUserProfile();
			...
			async login(params) {
				...
				try {
					...
					let {
						nickName,
						avatarUrl
					} = await this.$API.home.get_userinfo(params);
